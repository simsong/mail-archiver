...
* Normalize the valid RFC 5322 `Date:` and every valid timestamp suffix in a
  ...
  for embedded `Date:` headers and standard quoted `On ... wrote:` dates. Use
  the most recent plausible candidate and record `body-embedded` as
  `date_source`, while retaining the original bytes.
