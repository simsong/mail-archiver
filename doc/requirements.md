...
is scored only on a bounded sample; the complete part is strictly decoded only
after ranking. Only a final unrecoverable fallback may use UTF-8 replacement. `ftfy` encoding repair is
